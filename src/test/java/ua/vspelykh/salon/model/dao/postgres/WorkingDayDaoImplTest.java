...
import static ua.vspelykh.salon.model.dao.postgres.DaoTestData.getTestDates;
import static ua.vspelykh.salon.model.dao.postgres.DaoTestData.getTestWorkingDay;
import static ua.vspelykh.salon.model.dao.postgres.SqlConstants.WorkingDays.*;
...
class WorkingDayDaoImplTest extends AbstractDaoTest {
    ...
    void getDayByUserIdAndDate() throws SQLException, DaoException {
        try (PreparedStatement statement = mockPrepareStatement()) {
            ...
            WorkingDay workingDay = mockWorkingDayDao.getByUserIdAndDate(ID_VALUE, DATE_VALUE.toLocalDate());
    ...
    void getWorkingDaysByUserId() throws SQLException, DaoException {
        try (PreparedStatement statement = mockPrepareStatement()) {
            when(statement.executeQuery()).thenReturn(mockResultSet);
            mockResultSetIfPresent();
            List<WorkingDay> workingDays = mockWorkingDayDao.getByUserId(ID_VALUE);
    ...
    void deleteWorkingDaysByUserIdAndDatesArray() throws SQLException {
        try (PreparedStatement statement = mockPrepareStatement()) {
            when(statement.executeQuery()).thenReturn(mockResultSet);
            assertDoesNotThrow(() -> mockWorkingDayDao.deleteByUserIdAndDatesArray(ID_VALUE, getTestDates()));
