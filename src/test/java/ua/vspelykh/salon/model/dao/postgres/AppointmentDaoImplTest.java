...
import static ua.vspelykh.salon.model.dao.postgres.SqlConstants.Appointments.*;
...
class AppointmentDaoImplTest extends AbstractDaoTest {
    ...
    void getFiltered() throws SQLException, DaoException {
        try (PreparedStatement statement = mockPrepareStatement()) {
            ...
            List<Appointment> appointments = mockAppointmentDao.getFiltered(filter, 5, 1);
    ...
    void getCountOfAppointments() throws SQLException, DaoException {
        try (PreparedStatement statement = mockPrepareStatement()) {
            when(statement.executeQuery()).thenReturn(mockResultSet);
            mockResultSetIfPresent();
            int count = mockAppointmentDao.getCountOfAppointments(getAppointmentFilter());
    ...
    void getFilteredCheckSql(LocalDate from, LocalDate to, AppointmentStatus status, PaymentStatus paymentStatus,
        ...
        try (PreparedStatement statement = mockPrepareStatement()) {
            when(statement.executeQuery()).thenReturn(mockResultSet);
            mockResultSetIfPresent();
            mockAppointmentDao.getFiltered(createAppointmentFilter(from, to, status, paymentStatus), 1, 5);
            verifyQuery(sqlExpected);
        }
    }

    private AppointmentFilter createAppointmentFilter(LocalDate from, LocalDate to, AppointmentStatus status, PaymentStatus paymentStatus) {
    ...
    void getCountOfAppointmentsCheckSql(LocalDate from, LocalDate to, AppointmentStatus status, PaymentStatus paymentStatus,
        ...
        try (PreparedStatement statement = mockPrepareStatement()) {
            when(statement.executeQuery()).thenReturn(mockResultSet);
            mockResultSetIfPresent();
            mockAppointmentDao.getCountOfAppointments(createAppointmentFilter(from, to, status, paymentStatus));
            verifyQuery(sqlExpected);
        }
    }
