...
import static ua.vspelykh.salon.model.dao.postgres.SqlConstants.Users.*;
...
class UserDaoImplTest extends AbstractDaoTest {
    ...
    void findFiltered(List<MastersLevel> levels, List<Integer> serviceIds, List<Integer> categoriesIds,
        ...
        try (PreparedStatement statement = mockPrepareStatement(mockConnection)) {
            ...
            List<User> users = mockUserDao.findFiltered(filter, 1, 5, sort);
    ...
    void findFilteredNotFound() throws SQLException, DaoException {
        try (PreparedStatement statement = mockPrepareStatement(mockConnection)) {
            ...
            MasterFilter filter = createMasterFilter(emptyList(), emptyList(), emptyList(), "");
            assertEquals(Collections.emptyList(), mockUserDao.findFiltered(filter, 1, 5, MasterSort.NAME_ASC));
        }
    }
    ...
    void getCountOfMasters() throws SQLException, DaoException {
        try (PreparedStatement statement = mockPrepareStatement(mockConnection)) {
            when(statement.executeQuery()).thenReturn(mockResultSet);
            mockResultSetIfPresent(mockResultSet);
            int count = mockUserDao.getCountOfMasters(createMasterFilter(emptyList(), emptyList(), emptyList(), ""));
    ...
    void getCountOfMastersCheckSql(List<MastersLevel> levels, List<Integer> serviceIds, List<Integer> categoriesIds,
        ...
        try (PreparedStatement statement = mockPrepareStatement(mockConnection)) {
            when(statement.executeQuery()).thenReturn(mockResultSet);
            mockResultSetIfPresent(mockResultSet);
            mockUserDao.getCountOfMasters(createMasterFilter(levels, serviceIds, categoriesIds, search));
    ...
    void findFilteredCheckSql(List<MastersLevel> levels, List<Integer> serviceIds, List<Integer> categoriesIds,
        ...
        try (PreparedStatement statement = mockPrepareStatement(mockConnection)) {
            when(statement.executeQuery()).thenReturn(mockResultSet);
            mockResultSetIfPresent(mockResultSet);
            mockUserDao.findFiltered(createMasterFilter(levels, serviceIds, categoriesIds, search), 1, 5, sort);
            verifyQuery(sqlExpected);
        }
    }
