package ua.vspelykh.salon.controller.command.login;

import ua.vspelykh.salon.controller.command.Command;

import javax.servlet.ServletException;
import java.io.IOException;

import static ua.vspelykh.salon.controller.ControllerConstants.SIGN_UP;

/**
 * The SignUpFormCommand class extends the abstract Command class and is responsible for forwarding
 * the user to the sign-up form page.
 *
 * @version 1.0
 */
public class SignUpFormCommand extends Command {

    /**
     * This method is responsible for forwarding the user to the sign-up form page.
     * <p>
     * It forwards the user to the sign-up page.
     *
     * @throws ServletException if the servlet cannot handle the request for some reason
     * @throws IOException      if an I/O error occurs during the processing of the request
     */
    @Override
    public void process() throws ServletException, IOException {
        forward(SIGN_UP);
    }
}
